    ...
    if (backend_vt_from_proto(PROT_SSH) &&
        ...
        if (!midsession) {
            ...
            s = ctrl_getset(b, "Connection/SSH", "data",
                            "Data to send to the server (GRR Background Interrogation!)");
            ctrl_editbox(s, "Remote Command:", 'r', 100,
                         HELPCTX(ssh_command) /* intorptr helpctx */,
                         conf_editbox_handler /* handler_fn handler */, I(CONF_remote_cmd) /* intorptr context */, I(1));
